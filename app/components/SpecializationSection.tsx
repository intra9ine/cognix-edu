...
const SpecializationSection = () => {
  ...
  return (
      ...
      <main className="w-full mx-auto flex items-center justify-center flex-col">
        ...
        <ul className="grid grid-cols-3 gap-4 lg:gap-3 justify-items-center md:grid-cols-2 smx:grid-cols-1 w-full smx:mx-auto ">
          {specializations.map((item) => (
            <li
              ...
              className={`  ${hoveredItemId===item.id?'special-card':''}  h-[29rem] xlg:w-[19rem] xlg:h-[19rem] p-3  flex justify-center gap-4 items-center flex-col  transition-all duration-300  `}
            >
              ...
              <h6 className="w-3/4 text-[var(--darker-grey-hex)] lg:w-[65%] xlg:text-xs mx-auto text-center">
                {item.description}
              </h6>
            </li>
          ))}
